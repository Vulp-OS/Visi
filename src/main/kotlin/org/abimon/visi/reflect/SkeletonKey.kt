...
class SkeletonKey<T: Any>(private val klass: KClass<out T>, private val door: T?) {
    constructor(door: T): this(door::class, door)
    private val properties = klass.java.declaredFields.map { field -> field.name to field.apply { isAccessible = true } }.toMap()
    private val methods = klass.java.declaredMethods.map { method -> method.apply { isAccessible = true} }.groupBy { method -> method.name }
